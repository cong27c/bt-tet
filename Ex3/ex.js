const tarotDeck = [
 ...
 function createTarot() {
    ...
    btnMore.className = "btn btn-more"
    btnMore.textContent = "Rút thêm"

    const btnEnd = document.createElement("button")
    btnEnd.className = "btn btn-end"
    btnEnd.textContent = "Kết thúc"
...
function handle() {
      if(copyTarotDeck.length === 0) {
        if(confirm("Hết bài gòi:)) Bạn muốn có muốn kết thúc ko ??")) {
          copyTarotDeck = [...tarotDeck]
    ...
    const newCardContainer = document.createElement("div")
        ...
        let index = Math.floor(Math.random() * copyTarotDeck.length)
        const newSubTitle = document.createElement("h2")
        newSubTitle.className = "sub-title"
        newSubTitle.textContent = copyTarotDeck[index].name

        const newDesc = document.createElement("p")
        newDesc.className = "desc"
        newDesc.textContent = copyTarotDeck[index].description

        newCardContainer.append(newSubTitle, newDesc)
        cardContainer.appendChild(newCardContainer) 

        copyTarotDeck.filter( (card, index) => {
          if(card.name === newSubTitle.textContent) {
            copyTarotDeck.splice(index, 1)
          }
        })
}
...
btnDrawCard.addEventListener("click", handle)
   ...
   btnMore.addEventListener("click",handle)
    ...
    btnEnd.onclick = function() {
        if(confirm("Bạn thực sự muốn kết thúc chứ?")) {
          copyTarotDeck = [...tarotDeck]
          const newCardContainer = document.querySelectorAll(".new-card")
        newCardContainer.forEach(card => card.remove())
        }
